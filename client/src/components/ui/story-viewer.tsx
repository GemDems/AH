...
function StoryViewerModal({
  ...
  // Seamlessly switch products without unmounting: reset internal state when productKey changes
  React.useEffect(() => {
    if (productKey === undefined) return;
    setCurrentIndex(initialIndex);
    setProgress(0);
    setLocalViewedIndices(new Set());
    setDirection(0);
  ...
  React.useEffect(() => {
    ...
    // Images are instant — no load gate needed. Only videos wait for canPlay.
    setIsVideoReady(currentStory?.type === "image");
