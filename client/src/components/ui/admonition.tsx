...
export function Admonition({ type = "note", title, children, icon: CustomIcon, className = "" }: AdmonitionProps) {
  ...
  return (
    <div className={`${config.bgColor} ${config.borderColor} border rounded-lg p-4 ${className}`.trim()}>
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          {title && (
            <div className={`${config.textColor} font-semibold text-sm mb-1`}>{title}</div>
          )}
